...
namespace Bonwerk.SnooStudy
{
    public static class StudyPages
    {
        ...
        private static Image CreateAccuracyChart(FileLinker linker, StudyData data, string subName, 
            string studyName)
        {
            var plot = new Plot();
            plot.Title($"{subName} {studyName} Accuracy", fontSize: ProgramConfig.TitleSize);
            plot.PlotScatter(data.DailyOADate, data.DailyRSq, ProgramConfig.RSqColor, 1, 5, "R²");
            plot.PlotScatter(data.DailyOADate, data.DailyAccuracy, ProgramConfig.AccuracyColor, 2, 5, "Accuracy");
